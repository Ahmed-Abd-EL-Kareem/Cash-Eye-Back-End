...
// ── Match a hotel name mentioned in free text against cached search results ──
// Uses token-overlap matching so partial names (e.g. "Kempinski") still work,
// not just exact full-name substring matches. Shared by the pre-loop check
// (matches a PREVIOUS turn's search results against the new message) and the
// post-search check (matches the message that just triggered THIS turn's
// search — needed because search_hotels often runs in the same turn the
// user first names the hotel, so lastSearchResults isn't populated yet when
// the pre-loop check runs).
...
async function bookingAgentNode(state) {
  ...
  // capture its real id immediately — don't wait for the LLM to figure it out.
  if (!session.context.selectedHotelId && session.context.lastSearchResults?.length) {
    const bestMatch = matchHotelFromText(session.context.lastSearchResults, state.userMessage);
  ...
  for (let i = 0; i < 8; i++) {
    ...
    if (hasSearch) {
      ...
      // The user often names the hotel in the SAME message that triggers
      // this search (e.g. "book the Kempinski Nile Hotel Cairo"), so try
      // matching again now that lastSearchResults actually exists — the
      // pre-loop check above ran too early to see these results.
      if (!session.context.selectedHotelId && session.context.lastSearchResults?.length) {
        const postSearchMatch = matchHotelFromText(
          session.context.lastSearchResults,
          state.userMessage
        );
        if (postSearchMatch) {
          session.context.selectedHotelId = postSearchMatch.id;
          logger.info(
